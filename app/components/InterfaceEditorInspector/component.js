...
import InterfaceEditorInspectorTreeNavigator from '../InterfaceEditorInspectorTreeNavigator';
import InterfaceEditorInspectorStyleEditor from '../InterfaceEditorInspectorStyleEditor';
import InterfaceEditorInspectorPropsEditor from '../InterfaceEditorInspectorPropsEditor';
import InterfaceEditorInspectorTextEditor from '../InterfaceEditorInspectorTextEditor';
...
export default class InterfaceEditorInspector extends Component {
  ...
  getSelectedElementTypeDependentContentTabElement() {
    ...
    if (selectedElement.get('type') === Text) {
      return <InterfaceEditorInspectorTextEditor/>;
    }
  }
  ...
  renderSelectedTabComponent() {
    ...
    if (selectedTab === 0) {
      return <InterfaceEditorInspectorTreeNavigator/>;
    }

    if (selectedTab === 1) {
      return <InterfaceEditorInspectorStyleEditor/>;
    }

    if (selectedTab === 2) {
      return <InterfaceEditorInspectorPropsEditor/>;
    }
